...
class inkness_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'inkness_rp', // Base ID
			__('Inkness Recent Posts', 'inkness'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'inkness' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'inkness' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'inkness' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','inkness' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'inkness' ); ?></label>
...
add_action( 'widgets_init', 'register_inkness_widget' );  
function register_inkness_widget() {  
    register_widget( 'Inkness_Recent_Posts' );  
}  
